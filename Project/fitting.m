...
paths = ["Traces\TraceD-A.txt", "Traces\TraceD-V.txt", "Traces\TraceD-X.txt", "Traces\TraceD-C.txt"];

for i=1:4
    ServiceTime = csvread(paths(i));
    ...
    % Hypo-Exponential
    if(cv < 1)
        HypoE_values = mle(ServiceTime, 'pdf', @HypoExp_pdf, 'start', [1 / (0.3*M(1)), 1 / (0.7*M(1))], 'LowerBound', [0, 0], 'UpperBound', [Inf, Inf], 'Options', statset('MaxFunEvals', 1e5, 'MaxIter', 1e5));
